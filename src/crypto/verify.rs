...
///
/// The `verify_16()` function returns `true` iff `x[0], x[1], ..., x[15]`
/// are the same as `y[0], y[1], ..., y[15]`, `false` otherwise.
///
/// This function is safe to use for secrets `x[0], x[1], ..., x[15], y[0],
/// y[1], ..., y[15]`. The time taken by `verify_16()` is independent of
/// the contents of `x[0], x[1], ..., x[15], y[0], y[1], ..., y[15]`. In
...
///
/// ```
/// # use tweetnaclrs::crypto::verify;
/// let x = [1 as u8; 16];
/// let y = [2 as u8; 16];
/// if verify::verify_16(&x, &y) {
///     println!("Equal!");
/// } else {
///     println!("Not equal!");
/// }
...
///
/// The `verify_32()` function returns `true` iff `x[0], x[1], ..., x[31]`
/// are the same as `y[0], y[1], ..., y[31]`, `false` otherwise.
///
/// This function is safe to use for secrets `x[0], x[1], ..., x[31], y[0],
/// y[1], ..., y[31]`. The time taken by `verify_16()` is independent of
/// the contents of `x[0], x[1], ..., x[31], y[0], y[1], ..., y[31]`. In
